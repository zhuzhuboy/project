<template>
  <d2-container>
    ...
    <el-button slot="footer" type="danger" @click="handleClear">
      <d2-icon name="trash-o" />删除当前页面快照
    </el-button>
  </d2-container>
</template>
...
export default {
  ...
  methods: {
    ...mapActions("d2admin/db", ["pageSet", "pageGet", "pageClear"]),
    async handleClear() {
      await this.pageClear({ instance: this, user: true });
      this.$message.success(
        "此页面快照已经删除，请重新进入该页面或者关闭选项卡重新打开"
      );
    }
  }
};
