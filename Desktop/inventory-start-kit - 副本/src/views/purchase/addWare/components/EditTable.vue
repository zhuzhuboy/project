<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="one_price" label="采购单价">
        <template v-slot="{row}">{{row.one_price | NumFormat}}</template>
      </el-table-column>
...
export default {
    ...
    NumFormat: function(value) {
      ...
      if (value2Array.length === 2) {
        ...
        if (floatPart.length === 1) {
          // 补0,实际上用不着
          console.log(intPart + "." + floatPart + "0");
  ...
  methods: {
    ...
    tableClick(row, column, event) {
      if (column.property === "purchase_num") {
        // 子表格数量栏控制input显示
        row.purchase_num.show = true;
        this.inputFocus(event);
      } else if (column.property === "remarks") {
    ...
    className({ row, column, rowIndex, columnIndex }) {
      if (column.property === "purchase_num" || column.property === "remarks") {
        return "column-bg";
      }
    }
  }
